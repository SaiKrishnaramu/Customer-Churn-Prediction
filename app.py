...
def train_model(df: pd.DataFrame):
    ...
    # Permutation importance (as used in notebook — more reliable than built-in)
    perm_imp = permutation_importance(
        model, X_test, y_test, n_repeats=10, random_state=42, n_jobs=-1
    )
    feat_importances = perm_imp.importances_mean

    return model, X_test, y_test, y_prob, y_pred, auc, report, fpr, tpr, feat_importances
...
with st.spinner("Training model & computing permutation importance (cached after first run)…"):
    model, X_test, y_test, y_prob, y_pred, auc, report, fpr, tpr, feat_importances = train_model(df)
...
# ─────────────────────────────────────────────────────────────
    ...
    with col1:
        ...
        fig.update_layout(title="<b>ROC Curve</b>",
                          ...
                          legend=dict(x=0.55, y=0.15), height=420)
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        # Feature Importance — Permutation Importance (matches notebook)
        fi = (
        ...
        fig = px.bar(
            ...
            title="<b>Feature Importance (Permutation)</b>",
            color="Importance",
        ...
        fig.update_layout(
            ...
            height=420,
            xaxis_title="Mean Importance (permutation)",
            yaxis_title="",
        )
        st.plotly_chart(fig, use_container_width=True)
